...
public class CityQuarterReportStatExcelView extends AbstractExcelView{
    private final ModelAreaQuarterItem item;
    
	@Autowired
	CityQuarterReportStatExcelView(ModelAreaQuarterItem item){
		this.item=item;
	}
	@Override
	protected void buildExcelDocument(Map<String, Object> model,
		...
		String shellName;
		if(item!=null){
			shellName = item.getModelArea().getCityName()+"第"+item.getBatch().getBatchName()+"批市级新农村连片示范建设工程进展情况统计表";
		}else{
			shellName= "第一批市级新农村连片示范建设工程进展情况统计表";
		}
		// 设置标题样式
