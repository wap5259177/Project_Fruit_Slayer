...
public class ModelAreaHandleController extends AbstractController {
	...
	@Autowired
	public ModelAreaHandleController(NewRuralService newRuralService,
	...
	public Page base_village_search(@PathVariable("mid") String mid, Long id, HttpServletRequest request) {
		try {
			ReadEvent event = new ReadEvent(applicationContext, request,
					getUser());
			Finder fr = newRuralService.finder( event, NewRuralItem.class);
			/**
			 * fr.and("x.ownerId=?", getUser().getOwnerId());
			 */
			// 只选片区下的核心村
	...
	@RequestMapping(value = "!{mid}/village/search", method = {
			RequestMethod.GET, RequestMethod.POST })
	public Page village_search(@PathVariable("mid") String mid, Long id, HttpServletRequest request) {
		try {
			ReadEvent event = new ReadEvent(applicationContext, request,
					getUser());
	...
	public Page project_search(@PathVariable("mid") String mid,
		...
		try {
			...
			fr.and("areaName=? and status=?", e.getName(), 1);
			return fr.getPage();
	...
	public ModelAndView base_village_editor(HttpServletRequest request,
			@PathVariable("mid") String mid, Long areaId, Long ruralId) throws Exception {
		newId = ruralId;
...
//			model.addObject("vId", form.getPlaceId());
	...
	public ModelAndView project_editor(HttpServletRequest request,
		...
		if (null != id) {
			...
			form.set(entity);
//			model.addObject("area_name", form.getAreaName());
			model.addObject("id", id);
...
//			model.addObject("vId", form.getPlaceId());
	...
	//完成整治名单
	@ResponseBody
	@RequestMapping(value = "!{mid}/finishFix/editor", method = { POST, GET })
	public ModelAndView finishFix_editor(HttpServletRequest request,
			@PathVariable("mid") String mid, Long id, Long area_id) {
		DialogModel model = new DialogModel(mid + "editor", mid, request);
		addVillages(model);
		if (null != area_id) {
		...
		return model.execute("applicant/tab-finishFix-editor");
	}
...
//				form.toEntity(entity, modelAreaService.get(form.getModelareaId()));
	...
	@RequestMapping(value = "!{mid}/village/save", method = {
			RequestMethod.GET, RequestMethod.POST })
	public JsonResult village_save(@PathVariable("mid") String mid,PeripheraRuralEditor form, HttpServletRequest request) {
...
//				form.toEntity(entity, modelAreaService.get(form.getModelareaId()));
	...
	public JsonResult project_save(@PathVariable("mid") String mid,
		...
		try {
			...
			if (form.isNew()) {
				...
				entity.getInvestment().sumSelf();
				
				/*entity.getInvestment().setTotalFunds(
						entity.getInvestment().getStateFunds() + 
						entity.getInvestment().getProvinceFunds() + 
						//entity.getInvestment().getLocalFunds() + 
						entity.getInvestment().getSpecialFunds() + 
						entity.getInvestment().getCityFunds() +
						entity.getInvestment().getCountyFunds()+
						entity.getInvestment().getSocialFunds() + 
						entity.getInvestment().getMassFunds() + 
						entity.getInvestment().getOtherFunds());*/
				...
				/*entity.getInvestment().setTotalFunds(entity.getInvestment().getStateFunds() + entity.getInvestment().getProvinceFunds() 
						//+ entity.getInvestment().getLocalFunds()
						+ entity.getInvestment().getCityFunds() + entity.getInvestment().getCountyFunds() + entity.getInvestment().getMassFunds() 
						+ entity.getInvestment().getSpecialFunds() + entity.getInvestment().getSocialFunds() + entity.getInvestment().getOtherFunds());
				*/
	...
	public List<Item> levelNames() {
		List<ProjectTypeEntity> projectTypes = projectTypeService
				.find(" and x.ownerId=0");
		List<Item> listN = new ArrayList<Item>();

		if (null != projectTypes && projectTypes.size() != 0) {
			for (int i = 0; i < projectTypes.size(); i++) {
				ProjectTypeEntity e = projectTypes.get(i);
	 ...
	 */
	@ResponseBody
	@RequestMapping(value = "!{mid}/planning/editor", method = { POST, GET })
	public ModelAndView planning_editor(HttpServletRequest request,
			@PathVariable("mid") String mid, Long id, Long area_id) {
		DialogModel model = new DialogModel(mid + "editor", mid, request);
		// if(null != area_id) {
		// ModelAreaEntity e = modelAreaService.get(area_id);
		...
		// ProjectEditor form = new ProjectEditor();
		// form.set(entity);
		// model.addObject("area_name", form.getModelarea());
		// model.addObject("id", id);
		// model.addObject("form", form);
		// }
		return model.execute("applicant/temp");
	}
