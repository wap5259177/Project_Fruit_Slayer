...
@Controller
@RequestMapping("s/ml/area/closed")
public class ManagerModelAreaClosedController extends AbstractGridController<ModelAreaEntity, ModelAreaItem> {

	private ModelAreaService modelAreaService;
	
	@Autowired
	protected ManagerModelAreaClosedController(ModelAreaService modelAreaService) {
	...
	@QueryExpression("x.status=2 and cityId=(select y.place.id from UnitEntity y where y.id={USER owner})")
