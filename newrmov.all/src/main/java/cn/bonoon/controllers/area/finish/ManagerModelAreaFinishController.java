...
@Controller
@RequestMapping("s/ml/area/finish")
public class ManagerModelAreaFinishController extends AbstractGridController<ModelAreaEntity, ModelAreaItem> {

	private ModelAreaService modelAreaService;
	
	@Autowired
	protected ManagerModelAreaFinishController(ModelAreaService modelAreaService) {
	...
	@QueryExpression("x.status=1 and cityId=(select y.place.id from UnitEntity y where y.id={USER owner})")
